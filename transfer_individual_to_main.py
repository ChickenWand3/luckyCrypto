...
async def transfer_usdc(wallet, max_attempts=3):
    ...
    async with semaphore:
        try:
            ...
            nonce = await get_nonce(wallet["address"])
            gas_estimate = await estimate_gas(wallet["address"], balance)
            tx = await build_transaction(wallet["address"], nonce, gas_estimate, balance)
